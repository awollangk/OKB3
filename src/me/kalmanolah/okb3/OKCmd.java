...
public class OKCmd implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (is(label, "bbb")) {
			if ((args == null) || (args.length == 0) || (is(args[0], "help") && (args.length == 1)) || (is(args[0], "?") && (args.length == 1))) {
				handled = true;
				...
				if (OKmain.CheckPermission(getPlayer(sender), "bbb.force") || OKmain.CheckPermission(getPlayer(sender), "bbb.forceall") || OKmain.CheckPermission(getPlayer(sender), "bbb.ban") || OKmain.CheckPermission(getPlayer(sender), "bbb.unban")
						|| OKmain.CheckPermission(getPlayer(sender), "bbb.promote") || OKmain.CheckPermission(getPlayer(sender), "bbb.demote") || OKmain.CheckPermission(getPlayer(sender), "bbb.reload")
						|| OKmain.CheckPermission(getPlayer(sender), "bbb.rank")) {
				...
				if (isPlayer(sender)) {
					...
					OKConfig.loadkeys();
					OKLogger.info("Bulletin Board Bridge configuration file reloaded.");
				}
			} else {
				handled = true;
		...
		if (is(label, "sync")) {
			if (((Integer) OKFunctions.getConfig("mode") == 1) && (args.length == 2)) {
				handled = true;
				if (isPlayer(sender)) {
					String encpass = OKFunctions.getEncPass(args[0], args[1]);
					if (encpass.equals("nope")) {
						sendMessage(sender, ChatColor.RED + "Error: " + ChatColor.GRAY + "Incorrect username or password.");
					} else {
						OKFunctions.updateSecure(getPlayer(sender), getPlayer(sender), getName(sender), args[0], encpass, false);
					}
				}
			} else if (((Integer) OKFunctions.getConfig("mode") == 0) && (args.length == 0)) {
				handled = true;
				if (isPlayer(sender)) {
					OKFunctions.updateNormal(getPlayer(sender), getPlayer(sender), getPlayer(sender).getName(), false);
				}
			} else {
				handled = true;
				sendMessage(sender, "Incorrect command usage: /" + label + " " + join(args, 0));
			...
			if (args.length == 0) {
				...
				if (isPlayer(sender)) {
					...
					if (user == null) {
						...
						OKFunctions.updateSecure(getPlayer(sender), getPlayer(sender), getName(sender), user, pass, false);
					}
				}
			} else {
				handled = true;
				...
				if (isPlayer(sender)) {
					...
					try {
						if (!test.next()) {
							...
							OKLogger.info("[BANS] Unbanned player " + name + ".");
						}
						test.close();
					} catch (SQLException e) {
						e.printStackTrace();
									...
									if (track.get(newrankid) != null) {
										...
										if (OKmain.CheckPermission(getPlayer(sender), "bbb.promote." + String.valueOf(newrankid))) {
											...
											if (rankidentifiers.containsKey((newrank).toLowerCase())) {
												...
												sendMessage(sender, ChatColor.GOLD + "Notice: " + ChatColor.GRAY + "'" + ChatColor.WHITE + name + ChatColor.GRAY + "' was promoted to '" + ChatColor.WHITE + rankidentifiers.get(newrank)
														+ ChatColor.GRAY + "'.");
											} else {
